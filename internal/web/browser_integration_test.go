...
func TestBrowserLiveModeEndToEnd(t *testing.T) {
	...
	if err = chromedp.Run(browserContext,
		...
		chromedp.Evaluate(`(() => { const form=document.querySelector('#overview form[data-operation-relation="doctor"]'); const execution=document.createElement('article'); execution.id='synthetic-input-operation'; execution.className='execution'; execution.dataset.executionId='synthetic-prompt-execution'; execution.dataset.operationKey=form.dataset.operationKey; execution.dataset.status='waiting-input'; execution.dataset.statusLabel='Needs input'; execution.dataset.interactionLabel='Input required'; document.querySelector('#actions .action-list').prepend(execution); const dialog=document.createElement('dialog'); dialog.id='synthetic-input-dialog'; dialog.className='action-dialog interaction-dialog'; dialog.dataset.executionId='synthetic-prompt-execution'; dialog.dataset.operationKey=form.dataset.operationKey; dialog.dataset.dialogToken='synthetic-prompt'; dialog.innerHTML='<form class="prompt"><label>Response <input id="synthetic-prompt-input"></label><p class="prompt-feedback" role="status"></p><button type="submit">Submit response</button></form>'; document.querySelector('#action-results').prepend(dialog); })()`, nil),
		chromedp.Poll(`(() => { const form=document.querySelector('#overview form[data-operation-relation="doctor"]'); return document.querySelector('#synthetic-input-dialog')?.open && document.activeElement?.id === 'synthetic-prompt-input' && form?.dataset.operationState === 'waiting-input' && form.querySelector('.operation-button-text')?.textContent === 'Input required'; })()`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`(() => { const dialog=document.querySelector('#synthetic-input-dialog'); dialog.outerHTML=dialog.outerHTML; })()`, nil),
		chromedp.Poll(`document.querySelector('#synthetic-input-dialog')?.open && document.activeElement?.id === 'synthetic-prompt-input'`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`document.querySelector('#synthetic-input-dialog .prompt').dispatchEvent(new Event('submit', {bubbles:true, cancelable:true}))`, nil),
		chromedp.Poll(`(() => { const button=document.querySelector('#synthetic-input-dialog .prompt button'); return button?.disabled && button.textContent === 'Submitting…'; })()`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`globalThis.dwPromptRejected(document.querySelector('#synthetic-input-dialog .prompt'))`, nil),
		chromedp.Poll(`(() => { const button=document.querySelector('#synthetic-input-dialog .prompt button'); return !button?.disabled && button.textContent === 'Submit response' && document.querySelector('#synthetic-input-dialog .prompt-feedback')?.textContent === 'Response was not accepted. Try again.'; })()`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`(() => { const source=document.querySelector('#synthetic-input-operation'); const execution=source.cloneNode(false); execution.id='synthetic-input-operation-2'; execution.dataset.executionId='synthetic-prompt-execution-2'; document.querySelector('#actions .action-list').append(execution); const dialog=document.createElement('dialog'); dialog.id='synthetic-input-dialog-2'; dialog.className='action-dialog interaction-dialog'; dialog.dataset.executionId='synthetic-prompt-execution-2'; dialog.dataset.operationKey=source.dataset.operationKey; dialog.dataset.dialogToken='synthetic-prompt-2'; dialog.innerHTML='<form class="prompt"><label>Response <input id="synthetic-prompt-input-2"></label><button type="submit">Submit response</button></form>'; document.querySelector('#action-results').append(dialog); })()`, nil),
		chromedp.Evaluate(`(() => { const dialog=document.createElement('dialog'); dialog.id='synthetic-result-dialog'; dialog.className='action-dialog result-dialog'; dialog.dataset.executionId='synthetic-result-execution'; document.querySelector('#action-results').append(dialog); globalThis.dwOpenActionDialog('result','synthetic-result-execution'); })()`, nil),
		chromedp.Poll(`document.querySelector('#synthetic-input-dialog')?.open && document.querySelector('#synthetic-input-dialog-2')?.open === false && document.querySelector('#synthetic-result-dialog')?.open === false`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`document.querySelector('#synthetic-input-dialog').close()`, nil),
		chromedp.Poll(`document.querySelector('#synthetic-input-dialog-2')?.open && document.activeElement?.id === 'synthetic-prompt-input-2' && document.querySelector('#synthetic-result-dialog')?.open === false`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`document.querySelector('#synthetic-input-dialog-2').close()`, nil),
		chromedp.Poll(`document.querySelector('#synthetic-result-dialog')?.open`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`document.querySelector('#synthetic-result-dialog').close()`, nil),
		chromedp.Evaluate(`new Promise(resolve => { const marker=document.createElement('i'); document.body.append(marker); marker.remove(); setTimeout(() => resolve(document.querySelector('#synthetic-input-dialog')?.open !== true && document.querySelector('#synthetic-input-dialog-2')?.open !== true), 100); })`, nil),
		chromedp.Evaluate(`document.querySelector('#synthetic-input-operation')?.remove(); document.querySelector('#synthetic-input-dialog')?.remove(); document.querySelector('#synthetic-input-operation-2')?.remove(); document.querySelector('#synthetic-input-dialog-2')?.remove(); document.querySelector('#synthetic-result-dialog')?.remove()`, nil),
		...
		chromedp.Evaluate(`(() => { const form=document.querySelector('#overview form[data-operation-relation="doctor"]'); const execution=document.createElement('article'); execution.id='synthetic-auth-operation'; execution.className='execution'; execution.dataset.executionId='synthetic-auth-execution'; execution.dataset.operationKey=form.dataset.operationKey; execution.dataset.status='running'; execution.dataset.statusLabel='Running'; execution.dataset.interactionLabel='Sign-in required'; document.querySelector('#actions .action-list').prepend(execution); const dialog=document.createElement('dialog'); dialog.id='synthetic-auth-dialog'; dialog.className='action-dialog interaction-dialog'; dialog.dataset.executionId='synthetic-auth-execution'; dialog.dataset.operationKey=form.dataset.operationKey; dialog.dataset.dialogToken='synthetic-auth'; dialog.innerHTML='<a class="auth-action" href="#">Continue with Microsoft</a>'; document.querySelector('#action-results').prepend(dialog); })()`, nil),
		chromedp.Poll(`document.querySelector('#synthetic-auth-dialog')?.open && document.activeElement?.classList.contains('auth-action') && document.querySelector('#overview form[data-operation-relation="doctor"] .operation-button-text')?.textContent === 'Sign-in required'`, nil, chromedp.WithPollingMutation()),
		chromedp.Evaluate(`document.querySelector('#synthetic-auth-dialog').close(); document.querySelector('#synthetic-auth-operation').remove(); document.querySelector('#synthetic-auth-dialog').remove()`, nil),
		chromedp.Evaluate(`(() => { const toast=document.createElement('button'); toast.id='synthetic-expiring-toast'; toast.className='toast'; toast.dataset.expiresAt=String(Date.now()+50); document.querySelector('#action-notifications').append(toast); })()`, nil),
		chromedp.Poll(`document.querySelector('#synthetic-expiring-toast') === null`, nil, chromedp.WithPollingMutation()),
