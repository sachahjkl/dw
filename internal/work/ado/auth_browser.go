...
func (a *Authenticator) loginBrowser(ctx context.Context, timeout time.Duration, open bool, onInstructions func(BrowserLoginInstructions) error) (Token, error) {
	...
	authorizationURL := "https://login.microsoftonline.com/" + url.PathEscape(tenant) + "/oauth2/v2.0/authorize"
	query := url.Values{
		"client_id":             {clientID},
		"response_type":         {"code"},
	...
	if err := a.postBrowserTokenForm(ctx, "https://login.microsoftonline.com/"+url.PathEscape(tenant)+"/oauth2/v2.0/token", form, &token); err != nil {
		return Token{}, err
	}
...
func newBrowserCallbackServer(expectedState string, result chan<- callbackResult) *http.Server {
	server := &http.Server{ReadHeaderTimeout: 10 * time.Second, ReadTimeout: 30 * time.Second, WriteTimeout: 30 * time.Second}
	...
	server.Handler = http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		callback := parseCallback(query, expectedState)
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		if callback.Err != nil && query.Get("error") == "" {
