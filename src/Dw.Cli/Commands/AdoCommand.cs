...
internal static class AdoCommand
{
    ...
    private static int WorkItem(CommandContext context, string[] args)
    {
        ...
        var (workflow, azureDevOps, token) = CreateClientInputs(context, args);
        using var http = new HttpClient();
        var client = new AzureDevOpsClient(http, azureDevOps);
    ...
    private static (WorkflowConfig Workflow, AzureDevOpsOptions AzureDevOps, TokenResult Token) CreateClientInputs(CommandContext context, string[] args)
    {
        ...
        return (workflow, azureDevOps, token);
    }
    ...
    private static void PrintRelations(CommandContext context, JsonElement root, string[] args)
    {
        ...
        foreach (var relation in relations.EnumerateArray())
        {
            ...
            {
                context.Out.WriteLine($"  {StripMarkup(comment)}");
            }
        ...
        do
        {
            ...
            if (comments.RootElement.TryGetProperty("comments", out var array) && array.ValueKind == JsonValueKind.Array)
            {
                foreach (var comment in array.EnumerateArray())
                {
                    ...
                    context.Out.WriteLine(StripMarkup(text));
                    context.Out.WriteLine();
                    if (printed >= limit)
                    {
                        break;
    ...
    private static void PrintLongField(CommandContext context, string label, string? value)
    {
        ...
        context.Out.WriteLine(string.IsNullOrWhiteSpace(value) ? "(vide)" : StripMarkup(value));
        context.Out.WriteLine();
    }
    ...
    private sealed record AdoArtifactLink(string Display)
    {
