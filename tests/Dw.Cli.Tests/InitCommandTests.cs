...
public sealed class InitCommandTests
{
    ...
    public void Init_writes_config_and_embedded_schemas_with_relative_schema_links()
    {
        ...
        try
        {
            ...
            var exitCode = InitCommand.Run(context, new InitRequest(root, "business", NoSave: true, DryRun: false));
            ...
            Assert.Contains("follow the local `AGENTS.md`", opencodeInstructions);
            ...
            Assert.DoesNotContain("dw task current", opencodeInstructions);
            Assert.DoesNotContain("dw task commit", opencodeInstructions);
