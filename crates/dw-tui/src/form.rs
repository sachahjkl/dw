...
mod tests {
    ...
    fn task_start_pr_form_builds_preview_and_execute_actions() {
        ...
        snapshot.projects.projects.insert(
            "acme".into(),
            serde_json::json!({"displayName": "Acme", "repositories": {}}),
        );
        let mut form = FormState::selecting();
        ...
        match &preview.request {
            TuiActionRequest::TaskStartPr(args) => {
                ...
                assert_eq!(args.project.as_str(), "acme");
                assert_eq!(
                    args.repositories
                        .first()
    ...
    fn db_schema_form_uses_selected_database_defaults() {
        ...
        assert!(matches!(
            ...
            TuiActionRequest::DbSchema(ref args)
                if args.project.as_ref().map(dw_core::ProjectKey::as_str) == Some("acme")
                    && args.database.as_ref().map(dw_core::DatabaseKey::as_str) == Some("acme-dev")
        ));
    ...
    fn ado_set_state_form_builds_confirmed_background_action() {
        ...
        snapshot
            .projects
            .projects
            .insert("acme".into(), serde_json::json!({"displayName": "Acme"}));
        let mut form = FormState::selecting();
        form.template_index = FormTemplate::ALL
            .iter()
    ...
    fn suggestion_cycles_project_values_from_config() {
        ...
        assert_eq!(form.apply_suggestion(&snapshot).as_deref(), Some("acme"));
    }
    ...
    fn selected_suggestion_previews_next_text_value_only() {
        ...
        assert_eq!(form.selected_suggestion(&snapshot).as_deref(), Some("acme"));
    ...
    fn suggestion_reads_secret_keys_from_database_credentials() {
        ...
        assert_eq!(suggestions, vec!["db/shared", "db/acme-dev"]);
    }
    ...
    fn start_pr_form_prefills_and_suggests_loaded_pull_requests() {
        ...
        assert_eq!(
            field_value(&form.fields, "Project").as_deref(),
            Some("acme")
        );
        assert_eq!(
