...
mod tests {
  ...
  "projects": {
    ...
    fn typed_action_kind_is_separate_from_display_label() {
        let action = TuiAction {
            label: "Create workspace · acme #42".into(),
        ...
        assert_eq!(action.display_label(), "Create workspace · acme #42");
    }
    ...
    fn snapshot_exposes_assigned_work_item_prompt_specs_for_tui_forms() {
        ...
        snapshot.assigned = vec![AdoAssignedProject {
            key: "acme".into(),
            label: "acme - Acme Application".into(),
            items: vec![AdoAssignedItem {
                id: "114".into(),
                kind: "Task".into(),
                state: "Actif".into(),
                title: "Example synchronization".into(),
                url: None,
            }],
            error: None,
        }];
        ...
        assert_eq!(specs[0].id.as_str(), "assigned-work-item:acme");
        assert_eq!(specs[0].choices[0].value.as_str(), "114");
        assert_eq!(
            specs[0].choices[0].label,
            "#114 [Task] (Actif) Example synchronization"
        );
        assert_eq!(
  ...
  "projects": {
    "acme": {
      "displayName": "Acme",
      "azureDevOps": { "organization": "https://dev.azure.com/acme", "project": "Acme" },
      "repositories": {
        "front": {
          "url": "",
      ...
      "repositories": {
        ...
        assert_eq!(
            targets
                ...
                .collect::<Vec<_>>(),
            ["acme:front", "acme:back", "ops:tools"]
        );
        assert_eq!(
            targets
                .iter()
  ...
  "projects": {
    ...
    "cross-acme-suite": {
      ...
      "repositories": {
        ...
        "tools": {
          "url": "",
          "defaultBranch": "develop",
          "azureDevOpsRepository": "Acme Tools"
        }
      }
    }
  }
    ...
    fn pull_request_error_keeps_action_context() {
        ...
        assert_eq!(item.project, ProjectKey::from("acme"));
        assert_eq!(item.repository, WorkspaceRepositoryName::from("front"));
        assert_eq!(item.ado_repository, AdoRepositoryName::from("Acme Front"));
