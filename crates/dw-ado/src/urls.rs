...
mod tests {
    ...
    fn pull_request_web_url_targets_azure_devops_page_not_api_json() {
        let options = AzureDevOpsOptions {
            ...
            project: "Acme Application".into(),
            api_version: "7.1".into(),
        };

        let url = pull_request_web_url(&options, "front app", 114);

        assert_eq!(
            url,
            "https://dev.azure.com/acme/Acme%20Application/_git/front%20app/pullrequest/114"
        );
