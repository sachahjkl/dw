...
pub fn action_result_lines(result: &DwActionResult, theme: &TerminalTheme) -> Vec<String> {
    match result {
        ...
        DwActionResult::Task(result) => match result.as_ref() {
            ...
            TaskActionResult::DoingPlan(report) => task_doing_plan_lines(report),
            TaskActionResult::DoingExecution(report) => task_doing_execution_lines(report),
...
pub fn task_doing_plan_lines(report: &dw_task::doing::DoingPlanReport) -> Vec<String> {
    let mut lines = vec!["Task doing".into(), format!("Project: {}", report.project)];
    lines.extend(report.updates.iter().map(|update| {
        format!(
            "#{} [{}] {} -> {}{}",
            update.id,
            update.kind,
            ...
            update.target_state,
            if update.changed { "" } else { " (already set)" }
        )
    }));
    lines
}

pub fn task_doing_execution_lines(report: &dw_task::doing::DoingExecutionReport) -> Vec<String> {
    if report.updated.is_empty() {
        return vec!["No state change required.".into()];
    }
    report
        .updated
        .iter()
        .map(|update| format!("#{} -> {}", update.id, update.state))
        .collect()
}
...
fn doctor_remediation_label(remediation: &DoctorRemediation) -> String {
    match remediation {
        ...
        DoctorRemediation::InstallNodePackageManager => {
            "Install pnpm, or Node.js/npm when pnpm is unavailable.".into()
        }
...
mod tests {
    ...
    fn ado_prs_lines_include_work_items_and_branches() {
        ...
        assert_eq!(lines[0], "Active PRs · acme");
    ...
    fn ado_assigned_lines_render_start_action() {
        ...
        assert!(lines.contains(&"Start: start action for 42 (acme)".into()));
    }
    ...
    fn ado_assigned_group_lines_render_parent_children_and_start_action() {
        ...
        assert!(lines.contains(&"Start: start action for 42,43 (acme)".into()));
    ...
    fn ado_set_state_execution_lines_include_ids_and_state() {
        ...
        assert!(lines.contains(&"Project   : acme".into()));
    ...
    fn ado_work_item_lines_keep_context_action() {
        ...
        assert!(lines.contains(&"Context   : action available for #7 (acme)".into()));
    }
    ...
    fn ado_context_lines_include_relations_comments_and_ai_context_action() {
        ...
        assert!(output.contains("AI context: action available for #42 (acme)"));
    }
    ...
    fn task_preflight_lines_include_blocking_guidance() {
        let report = TaskPreflightReport {
            ...
            project: dw_core::ProjectKey::from("acme"),
    ...
    fn task_prune_plan_lines_render_preview_summary() {
        let report = dw_task::prune::PrunePlanReport {
            ...
            candidates: vec![dw_workspace::WorkspaceSummary {
                path: "/tmp/dw/projects/acme/workspaces/feat-1-done".into(),
    ...
    fn task_start_plan_lines_include_execute_hint() {
        let report = dw_task::start::StartPlanReport {
            ...
            plan: dw_workspace::TaskStartPlan {
                ...
                workspace: dw_core::WorkspacePath::from("/tmp/dw/acme/42-titre"),
                repositories: vec![
