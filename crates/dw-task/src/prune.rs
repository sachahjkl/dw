...
pub struct PruneArgs {
    ...
    pub selected_workspaces: Option<Vec<WorkspacePath>>,
...
mod tests {
    ...
    async fn prune_no_sync_dry_run_does_not_require_auth() {
        ...
        let workspace = root.join("projects/acme/workspaces/feat-1-done");
        std::fs::create_dir_all(&workspace).expect("workspace should be created");
        std::fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"1","taskId":null,"project":"acme","type":"feat","slug":"done","branchName":"feat/1-done","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created","workItems":[{"id":"1","type":"User Story","title":"Done","state":"Valide"}]}"#,
        )
    ...
    fn prune_candidate_label_includes_project_and_items() {
        ...
        assert_eq!(prune_candidate_label(&candidate), "acme / #1 Done [Valide]");
    }
    ...
    fn prune_candidate_choice_includes_context_and_path() {
        ...
        assert_eq!(
            ...
            "acme / #1 Done [Valide] - front, back - /tmp/dw/projects/acme/workspaces/feat-1-done"
        );
    }
