...
async fn handle_work(command: WorkCommand) -> Result<()> {
    match command {
        ...
        WorkCommand::Item {
            ...
            confirm_task_doing(yes, &plan)?;
            let result = execute_cli_action_with_event_output(
                dw_app::DwActionRequest::TaskDoingExecute(plan),
                !json,
            )
            .await?;
            let dw_app::DwActionResult::Task(result) = result else {
            ...
            if json {
                ...
                print_lines(&dw_cli_adapter::render::task_doing_execution_lines(
                    &execution,
                ));
            }
        }
...
fn confirm_task_doing(yes: bool, plan: &dw_task::doing::DoingPlanReport) -> Result<()> {
    if yes || plan.updates.iter().all(|update| !update.changed) {
        return Ok(());
    }
    if !std::io::stdin().is_terminal() {
    ...
    let changed = plan
        ...
        .map(|update| format!("#{} -> {}", update.id, update.target_state))
        .collect::<Vec<_>>()
        .join("\n");
    let mut prompt_ui = InquirePrompt;
    if prompt_ui.confirm(
        &confirm_risk_prompt_spec(
            "task-doing",
            format!("Move these work items to their in-progress state?\n{changed}"),
        ),
        false,
    )? {
        Ok(())
...
mod prompt_tests {
    ...
    fn prompt_choice_value_returns_typed_value_not_label_text() {
        let spec = PromptSpec::select(
            ...
            vec![dw_core::PromptChoice::new(
                "114",
                "#114 [Task] (Active) Automatic transmission",
            )],
        );

        let value =
            prompt_choice_value_from_label(&spec, "#114 [Task] (Active) Automatic transmission")
                .expect("choice should resolve");

        assert_eq!(value.as_str(), "114");
    }
    ...
    fn progress_messages_are_human_readable_not_action_ids() {
        ...
        assert_eq!(
            message,
            "Building workspace plan for acme across 2 repositories..."
        );
    ...
    fn task_start_execute_args_preserve_resolved_interactive_plan() {
        let report = dw_task::start::StartPlanReport {
            ...
            plan: dw_workspace::TaskStartPlan {
                work_item_ids: vec![WorkItemId::from("119")],
                primary_work_item_id: WorkItemId::from("119"),
                project: ProjectKey::from("acme"),
                task_id: None,
                kind: WorkItemTypeName::from("feat"),
                slug: TaskSlug::from("gestion-retour-succes"),
                branch_name: dw_core::BranchName::from("feat/119-gestion-retour-succes"),
                subject_name: dw_core::TaskSubjectName::from("feat-119-gestion-retour-succes"),
        ...
        assert_eq!(args.work_item_ids, vec![WorkItemId::from("119")]);
        assert_eq!(args.project, Some(ProjectKey::from("acme")));
