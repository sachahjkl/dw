...
fn update_handoff_child_tasks(
    workspace: &WorkspacePath,
    repository: &str,
    child_tasks: &[WorkspaceChildTask],
) -> Result<(), WorkspaceError> {
    let path = Path::new(workspace.as_str()).join(format!("handoff-{repository}.md"));
    let Ok(contents) = fs::read_to_string(&path) else {
        return Ok(());
    };
    let known = child_tasks
        .iter()
        .filter(|task| task.repository.as_str().eq_ignore_ascii_case(repository))
        .map(|task| match &task.title {
            Some(title) => format!("#{} ({title})", task.id),
            None => format!("#{}", task.id),
        })
        .collect::<Vec<_>>();
    ...
    let updated = contents
        .lines()
        .map(|line| {
            if line.starts_with("- Child tasks connus:") {
                replacement.as_str()
            } else {
                line
            }
...
pub fn execute_add_child_task(
    ...
    update_handoff_child_tasks(workspace, repository, &updated.normalized_child_tasks())?;
    Ok(updated)
}
...
mod tests {
    ...
    #[test]
    fn update_handoff_child_tasks_preserves_existing_content() {
        let temp = tempdir().expect("tempdir");
        let handoff = temp.path().join("handoff-back.md");
        fs::write(
            &handoff,
        ...
        update_handoff_child_tasks(
            &typed_workspace_path(temp.path()),
            "back",
            &[WorkspaceChildTask {
                repository: WorkspaceRepositoryName::from("back"),
                id: WorkItemId::from("116"),
                title: Some(WorkItemTitle::from("[BACK] Corriger la notification")),
            }],
        )
        .expect("update handoff");

        let updated = fs::read_to_string(handoff).expect("updated handoff");
        assert!(updated.contains("- Child tasks connus: #116 ([BACK] Corriger la notification)"));
...
"#;
    ...
    fn finds_and_filters_workspaces() {
        ...
        let root = temp.path();
        let workspace = test_workspace_dir(root, "acme", "feat-123-demo");
  ...
  "status": "created"
        ...
        assert_eq!(workspaces[0].manifest.project, ProjectKey::from("acme"));

        let filtered = filter_workspaces(workspaces, Some("acme"), Some("123"));
    ...
    fn find_workspaces_scans_only_workspace_directories() {
        ...
        let anchor = test_anchor_dir(root, "acme", "front.git");
        fs::create_dir_all(&repository).expect("repository should exist");
        fs::create_dir_all(&anchor).expect("anchor should exist");
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"123","taskId":null,"project":"acme","type":"feat","slug":"demo","branchName":"feat/123-demo","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("manifest should be written");
        fs::write(
            repository.join("task.json"),
            r#"{"schema":1,"workItemId":"999","taskId":null,"project":"acme","type":"feat","slug":"nested","branchName":"feat/999-nested","createdAt":"2026-07-03T10:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("nested manifest should be written");
        fs::write(
            anchor.join("task.json"),
            r#"{"schema":1,"workItemId":"998","taskId":null,"project":"acme","type":"feat","slug":"anchor","branchName":"feat/998-anchor","createdAt":"2026-07-04T10:00:00Z","repositories":["front"],"status":"created"}"#,
        )
  ...
  "status": "created"
    ...
    fn task_list_splits_legacy_comma_separated_repositories() {
        ...
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"123","taskId":null,"project":"acme","type":"feat","slug":"demo","branchName":"feat/123-demo","createdAt":"2026-07-02T10:00:00Z","repositories":["front,back"],"status":"created"}"#,
        )
        .expect("manifest should be written");
  ...
  "status": "created",
  "workItems": [
    { "id": "123", "type": "User Story", "title": "Example title", "state": "Active" }
  ]
}"#,
        )
        .expect("manifest should be written");

        let result = task_list(root.to_str().expect("utf8 path"), Some("acme"), Some("123"));
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].project.as_str(), "acme");
    ...
    fn resolves_current_workspace_from_nested_path() {
        let temp = tempdir().expect("tempdir should be created");
        let workspace = test_workspace_dir(temp.path(), "acme", "feat-123-demo");
  ...
  "status": "created"
        ...
        assert_eq!(current.project.as_str(), "acme");
  ...
  "status": "created"
    ...
    fn resolve_workspace_continue_uses_latest_matching_workspace() {
        let temp = tempdir().expect("tempdir should be created");
        let root = temp.path();
        let old_workspace = test_workspace_dir(root, "acme", "feat-1-old");
        let new_workspace = test_workspace_dir(root, "acme", "feat-2-new");
        ...
        fs::write(
            ...
            r#"{"schema":1,"workItemId":"1","taskId":null,"project":"acme","type":"feat","slug":"old","branchName":"feat/1-old","createdAt":"2026-06-20T00:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("old manifest should be written");
        fs::write(
            new_workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"2","taskId":null,"project":"acme","type":"feat","slug":"new","branchName":"feat/2-new","createdAt":"2026-06-21T00:00:00Z","repositories":["front"],"status":"created"}"#,
        )
    ...
    fn resolve_workspace_uses_positional_work_item_id() {
        let temp = tempdir().expect("tempdir should be created");
        let root = temp.path();
        let workspace = test_workspace_dir(root, "acme", "feat-101-new");
        fs::create_dir_all(&workspace).expect("workspace should exist");
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"101","taskId":null,"project":"acme","type":"feat","slug":"new","branchName":"feat/101-new","createdAt":"2026-06-21T00:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("manifest should be written");
    ...
    fn resolve_workspace_matches_secondary_work_item_id() {
        ...
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"101","taskId":null,"project":"acme","type":"feat","slug":"new","branchName":"feat/101-new","createdAt":"2026-06-21T00:00:00Z","repositories":["front"],"status":"created","workItems":[{"id":"101","type":"User Story","title":"Principal","state":"En réalisation"},{"id":"111","type":"Bug","title":"Secondaire","state":"En développement"}]}"#,
        )
        .expect("manifest should be written");
    ...
    fn resolve_open_target_returns_repo_folder() {
        ...
        let project_config: dw_config::ProjectConfig = serde_json::from_str(
            r#"{"displayName":"Acme","repositories":{"front":{"url":"","defaultBranch":"develop","folder":"custom-front"}}}"#,
        )
    ...
    fn plan_task_start_uses_default_project_type_and_fallback_slug() {
        ...
        let plan = plan_task_start(TaskStartRequest {
            ...
            projects: &projects,
            work_item_ids: &[WorkItemId::from("113")],
            project: None,
            task_id: None,
            type_name: None,
            repositories: &[],
        ...
        assert_eq!(plan.slug.as_str(), "work-item-113");
        assert_eq!(plan.branch_name.as_str(), "feat/113-work-item-113");
    }
    ...
    fn plan_task_start_rejects_existing_workspace_conflict() {
        ...
        .expect("manifest should be written");
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"front":{"url":"","defaultBranch":"develop"}}}}}"#,
        )
        .expect("projects should parse");
        ...
        let error = plan_task_start(TaskStartRequest {
            ...
            project: Some(&ProjectKey::from("acme")),
            task_id: None,
            type_name: Some("feat"),
            repositories: &[],
            slug: Some("demo"),
    ...
    fn plan_task_start_uses_repository_folder_override() {
        ...
        let root = temp.path();
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"front":{"url":"","defaultBranch":"develop","folder":"custom-front"}}}}}"#,
        )
        .expect("projects should parse");
    ...
    fn execute_task_start_rejects_unpreparable_repository() {
        let temp = tempdir().expect("tempdir should be created");
        let workspace = test_workspace_dir(temp.path(), "acme", "feat-123-demo");
        let project_root = test_project_dir(temp.path(), "acme");
  ...
  "projects": {{
    "acme": {{
      ...
      "repositories": {{
        ...
        let anchor = test_anchor_dir(&root, "acme", "front.git");
        let worktree = test_workspace_dir(&root, "acme", "feat-123-demo").join("front");
    ...
    fn execute_task_start_writes_manifest_plan_and_handoffs() {
        ...
        assert_eq!(manifest.project, ProjectKey::from("acme"));
        assert!(workspace.join("task.json").exists());
    ...
    fn preflight_report_models_warning_issues_from_ai_context_file() {
        let temp = tempdir().expect("tempdir should be created");
        let workspace = test_workspace_dir(temp.path(), "acme", "feat-109-demo");
        fs::create_dir_all(&workspace).expect("workspace should exist");
        fs::write(
            workspace.join("task.json"),
            r#"{
  ...
  "type": "feat",
  "slug": "demo",
  "branchName": "feat/109-demo",
  "createdAt": "2026-07-02T10:00:00Z",
  "repositories": ["front"],
  "status": "created",
  "workItems": [
    { "id": "109", "type": "Bug", "title": "Titre local", "state": "New" }
  ]
}"#,
        )
        .expect("manifest should be written");
  ...
  "workItem": {
    "id": "109",
    "url": "https://dev.azure.com/org/Project/_workitems/edit/109",
    "title": "Titre ADO",
    "type": "Bug",
  ...
  "attachments": {
    "directoryHint": "attachments/ado/109",
    "items": [
      { "name": "mockup.png", "url": "https://dev.azure.com/org/_apis/wit/attachments/123", "comment": "Example screen", "directoryHint": "attachments/ado/109" }
    ]
  },
  "relations": [],
  "comments": []
    ...
    fn plan_task_repo_latest_uses_default_branch_and_folder_override() {
        ...
        .expect("manifest should be written");
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"front":{"url":"","defaultBranch":"develop","folder":"custom-front"},"back":{"url":"","defaultBranch":"main"}}}}}"#,
        )
        .expect("projects should parse");
  ...
  "project": "example",
  "type": "bug",
  "slug": "fix-example-dialog",
  "branchName": "bug/103-105-109-fix-example-dialog",
  "createdAt": "2026-07-02T10:00:00Z",
  "repositories": ["front"],
  "status": "created",
  "childTasks": [
    { "repository": "front", "id": "109", "title": "Verifier UI" }
  ]
}"#,
        )
        .expect("manifest should parse");
        ...
        assert_eq!(
            ...
            "bug(#103 #105 #109): fix-example-dialog"
        );
    }
  ...
  "type": "bug",
  "slug": "fix-example-dialog",
  "branchName": "bug/103-120-fix-example-dialog",
  "createdAt": "2026-07-02T10:00:00Z",
  "repositories": ["back"],
  "status": "created",
  "workItems": [
    ...
    { "id": "120" }
        ...
        assert_eq!(
            build_commit_message(&manifest, None).as_str(),
            "bug(#103 #120): fix-example-dialog"
        );
    }
    ...
    fn build_commit_message_adds_reference_to_override_when_missing() {
        let manifest: WorkspaceManifest = serde_json::from_str(
            r#"{"schema":1,"workItemId":"102","taskId":"109","project":"acme","type":"feat","slug":"example","branchName":"feat/102-109-example","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("manifest should parse");

        assert_eq!(
            build_commit_message(&manifest, Some(&CommitMessage::from("feat: example"))).as_str(),
            "feat: example #109"
        );
        assert_eq!(
            build_commit_message(&manifest, Some(&CommitMessage::from("feat: example #102")))
                .as_str(),
            "feat: example #102"
        );
    }
    ...
    fn plan_add_work_items_updates_branch_and_workspace_subject() {
        let temp = tempdir().expect("tempdir should be created");
        let workspace = test_workspace_dir(temp.path(), "acme", "feat-101-demo");
        fs::create_dir_all(&workspace).expect("workspace should exist");
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"101","taskId":null,"project":"acme","type":"feat","slug":"demo","branchName":"feat/101-demo","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("manifest should be written");
        ...
        assert_eq!(plan.new_branch.as_str(), "feat/101-111-demo");
        assert!(plan.new_workspace.as_str().ends_with("feat-101-111-demo"));
    ...
    fn plan_add_work_item_snapshots_rejects_workspace_conflict() {
        let temp = tempdir().expect("tempdir should be created");
        let workspace = test_workspace_dir(temp.path(), "acme", "feat-101-demo");
        let other = test_workspace_dir(temp.path(), "acme", "bug-111-other");
        fs::create_dir_all(&workspace).expect("workspace should exist");
        ...
        fs::write(
            ...
            r#"{"schema":1,"workItemId":"111","taskId":null,"project":"acme","type":"bug","slug":"other","branchName":"bug/111-other","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        ...
        assert!(error.to_string().contains("bug-111-other"));
    }
    ...
    fn execute_work_item_update_writes_manifest_and_renames_workspace() {
        ...
        assert_eq!(updated.branch_name, BranchName::from("feat/101-111-demo"));
    ...
    fn plan_task_add_repo_uses_config_folder_anchor_and_branch() {
        ...
        .expect("manifest should be written");
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"db":{"url":"https://example.invalid/db.git","defaultBranch":"develop","anchorName":"database.git","folder":"database"}}}}}"#,
        )
        .expect("projects should parse");
    ...
    fn execute_task_add_repo_updates_manifest_and_handoff() {
        ...
        .expect("manifest should be written");
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"db":{"url":"","defaultBranch":"main"}}}}}"#,
        )
        .expect("projects should parse");
    ...
    fn execute_task_sync_updates_parent_work_items_from_snapshots() {
        ...
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"123","taskId":null,"project":"acme","type":"feat","slug":"demo","branchName":"feat/123-demo","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created","workItems":[{"id":"123","type":"Bug","title":"Ancien","state":"New"}]}"#,
        )
        .expect("manifest should be written");
    ...
    fn workspace_work_item_display_includes_title_and_state() {
        let text = WorkspaceWorkItem {
            id: "111".into(),
            kind: Some("Bug".into()),
            title: Some("Example values are inconsistent".into()),
            state: Some("Valide".into()),
        }
        .to_string();

        assert_eq!(text, "#111 Example values are inconsistent [Valide]");
    }
    ...
    fn plan_task_prune_keeps_only_workspaces_with_all_parents_final() {
        let temp = tempdir().expect("tempdir should be created");
        let root = temp.path();
        let final_workspace = test_workspace_dir(root, "acme", "feat-1-final");
        let active_workspace = test_workspace_dir(root, "acme", "feat-2-active");
        ...
        fs::write(
            ...
            r#"{"schema":1,"workItemId":"1","taskId":null,"project":"acme","type":"feat","slug":"final","branchName":"feat/1-final","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created","workItems":[{"id":"1","type":"User Story","title":"Done","state":"Valide"}]}"#,
        )
        .expect("final manifest should be written");
        fs::write(
            active_workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"2","taskId":null,"project":"acme","type":"feat","slug":"active","branchName":"feat/2-active","createdAt":"2026-07-02T10:00:00Z","repositories":["front"],"status":"created","workItems":[{"id":"2","type":"User Story","title":"Active","state":"En realisation"}]}"#,
        )
        .expect("active manifest should be written");

        let candidates = plan_task_prune(root.to_str().expect("utf8 path"), Some("acme"), None);
    ...
    fn plan_task_teardown_removes_each_repo_worktree_and_prunes_git_anchors() {
        let temp = tempdir().expect("tempdir should be created");
        let root = temp.path();
        let workspace = test_workspace_dir(root, "acme", "feat-113-slug");
        fs::create_dir_all(&workspace).expect("workspace should exist");
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"113","taskId":null,"project":"acme","type":"feat","slug":"slug","branchName":"feat/113-slug","createdAt":"2026-06-22T12:00:00Z","repositories":["front"],"status":"created"}"#,
        )
        .expect("manifest should be written");
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"front":{"url":"https://example.invalid/front.git","defaultBranch":"develop","anchorName":"front.git","folder":"front"}}}}}"#,
        )
        .expect("projects should parse");
    ...
    fn plan_task_teardown_splits_legacy_comma_separated_repositories() {
        ...
        fs::write(
            workspace.join("task.json"),
            r#"{"schema":1,"workItemId":"113","taskId":null,"project":"acme","type":"feat","slug":"slug","branchName":"feat/113-slug","createdAt":"2026-06-22T12:00:00Z","repositories":["front,back"],"status":"created"}"#,
        )
        .expect("manifest should be written");
        let projects: ProjectsConfig = serde_json::from_str(
            r#"{"projects":{"acme":{"displayName":"Acme","repositories":{"front":{"url":"https://example.invalid/front.git","defaultBranch":"develop","anchorName":"front.git","folder":"front"},"back":{"url":"https://example.invalid/back.git","defaultBranch":"main","anchorName":"back.git","folder":"back"}}}}}"#,
        )
        .expect("projects should parse");
